...
function parseInitialIntent(prompt) {
  ...
  recordAuditEvent({
    ...
    status: 'INFO',
    description: `Customer natural prompt parsed: "${prompt}". Extracted target category: ${result.category}, budget: ₹${result.budget.toLocaleString('en-IN')}, primary use: ${result.primaryUse}.`,
    payload: {
      rawPrompt: prompt,
...
function broadcastRfpAndEvaluateOffers(requirements = {}) {
  ...
  const evaluatedOffers = allOffers.map(offer => {
    ...
    let policyCheck = { status: 'PASS', reason: `Within hard budget limit of ₹${targetBudget.toLocaleString('en-IN')}.` };
    if (offer.finalTotal > targetBudget) {
      const overBy = offer.finalTotal - targetBudget;
      policyCheck = {
        status: 'BLOCKED',
        reason: `Policy Blocked: Exceeds customer maximum budget of ₹${targetBudget.toLocaleString('en-IN')} by ₹${overBy.toLocaleString('en-IN')}.`
      };
    }
  ...
  const whyNexa = `${recommendedPick.merchant} is recommended because it satisfies all hard constraints (under ₹${targetBudget.toLocaleString('en-IN')}) and provides superior overall value with ${recommendedPick.specs?.ram || '16GB RAM'}, ${recommendedPick.warranty || '12-Month Warranty'}, and ${recommendedPick.deliveryTime || '1-Day Delivery'}.`;
...
function runBoundedNegotiation(offerId = 'offer_01', requirements = {}) {
  ...
  const round1 = {
    ...
    buyerAgentAction: `Buyer Agent requests ₹${requestedConcession.toLocaleString('en-IN')} price concession.`,
    reason: `Product price of ₹${originalPrice.toLocaleString('en-IN')} exceeds customer's preferred budget target.`,
    policyCheck: {
      action: `₹${requestedConcession.toLocaleString('en-IN')} Discount Application`,
    ...
    merchantResponse: {
      status: 'APPROVED',
      message: `Counter-offer accepted with ₹${requestedConcession.toLocaleString('en-IN')} direct discount.`
    }
  };
  ...
  const round2 = {
    ...
    merchantResponse: {
      status: 'APPROVED',
      message: 'Approved 1-Day Guaranteed Express Delivery (₹499 value waived).'
    }
  };
  ...
  const round3 = {
    ...
    policyCheck: {
      ...
      marginCalculation: `Final Merchant Margin = ${marginAfter}% | Total Customer Savings = ₹${requestedConcession.toLocaleString('en-IN')}`,
      result: 'AGREEMENT LOCKED'
    },
    merchantResponse: {
      status: 'FINALIZED',
      message: `Final Authoritative Amount: ₹${authoritativeFinalTotal.toLocaleString('en-IN')}. Ready for customer authorization.`
    }
  };
  ...
  recordAuditEvent({
    ...
    status: 'SUCCESS',
    description: `Concession Allowed: ₹${requestedConcession.toLocaleString('en-IN')} (${discountPercent}%) discount within merchant ${maxConcession}% limit. Remaining margin: ${marginAfter}%.`,
    payload: {
...
function explainProduct(productId = 'prod_lap_01', requirements = {}) {
  ...
  const requirementMatches = [
    {
      requirement: `Hard Budget Limit: Under ₹${budgetCap.toLocaleString('en-IN')}`,
      productSpec: `₹${price.toLocaleString('en-IN')} Final Price`,
      explanation: price <= budgetCap
        ? `Fits comfortably under your budget with ₹${(budgetCap - price).toLocaleString('en-IN')} in headroom remaining.`
    ...
    },
    {
      ...
      productSpec: `${item.warranty || '12-Month On-Site Warranty'} • ${item.returnPolicy || '14-Day Free Returns'}`,
      explanation: `${merchant} is a certified brand partner with a 4.9/5 merchant rating across 3,800+ orders.`,
      status: 'Verified Match'
    }
  ];

  const summary = `NEXA selected the ${name} by ${merchant} because it achieves the highest Multi-Attribute Utility Score (92/100). It satisfies all hard budget constraints (₹${price.toLocaleString('en-IN')} vs ₹${budgetCap.toLocaleString('en-IN')} cap), provides fast 10-core compilation throughput with 16GB DDR5 RAM for ${primaryUse}, and includes 1-day express delivery plus ₹1,999 in free bundled developer accessories.`;

  const tradeoffs = {
    consideration: `Alternative models like the Acer Swift Go 14 feature OLED panels but cost ₹78,990 (exceeding your budget by ₹8,990). The ${name} provides the most balanced compromise between raw compilation power, thermal stability, and price.`
  };
...
function answerFollowUp(productId = 'prod_lap_01', question = '', requirements = {}) {
  ...
  // 2. Cheaper alternative / lower price
    ...
    field = 'budget';
    value = 60000;
    answer = `Yes! If you are looking for a more affordable alternative, I recommend the **Acer Aspire 5 Pro** (₹56,490 by AeroTech Hub) or **HP Pavilion 14** (₹58,990 by TechNova). Both provide 16GB RAM and Core i5 performance while saving you over ₹11,000 from your budget! I have updated your recommendations below.`;
  }
  // 3. Higher budget / ₹80,000
    ...
    field = 'budget';
    value = 80000;
    answer = `With the higher budget of ₹80,000, I found top options with better processors and OLED displays, such as the **Acer Swift Go 14 OLED AI** (Intel Core Ultra 5 / NPU at ₹78,990) and **Lenovo LOQ 15**. I've updated your recommendations!`;
  }
  // 4. Battery life
  ...
  // 5. Delivery tomorrow / speed
    ...
    if (item.deliveryDays && item.deliveryDays <= 1) {
      ...
      answer = `The ${name} standard delivery takes 2–3 business days. However, I found two alternative laptops with Guaranteed Next-Day Delivery (Lenovo IdeaPad Slim 5 & Acer Aspire 5 Pro).`;
    }
  }
  ...
  // 8. General fallback
    ...
    answer = `Regarding "${question}": The **${name}** (₹${price.toLocaleString('en-IN')}) is certified for high-reliability development and multitasking with ${item.specs?.ram || '16GB RAM'} and ${item.specs?.processor || 'Core i5 CPU'}. Feel free to ask about specific tools, software, or delivery terms!`;
  }
...
function getWhyNexaExplanation(decisionType, context = {}) {
  const explanations = {
    RECOMMENDATION: {
      ...
      inputFactors: ['Budget Cap: ₹70,000', '16GB DDR5 RAM', '1-Day Delivery SLA', '12-Month Warranty'],
      policyConstraints: ['Hard Budget <= ₹70,000', 'Risk Assessment = LOW'],
      expectedOutcome: 'Optimal developer performance and reliability with zero budget violation.'
    },
    DISCOUNT_CONCESSION: {
      decision: `Applied ₹${context.discount || '1,380'} Concession`,
      reason: 'Customer price sensitivity prioritized without violating merchant minimum 12% margin threshold.',
      inputFactors: ['Original Price: ₹68,990', 'Concession Limit: 8%', 'Minimum Margin: 12%'],
    ...
    POLICY_BLOCK: {
      ...
      inputFactors: ['Customer Budget: ₹70,000', 'Offered Price: ₹88,900', 'Over Budget: ₹18,900'],
...
function getMerchantAnalyticsData(merchantId = 'merch_technova') {
  ...
  return {
    ...
    topMetrics: {
      totalAiRevenue: '₹1.42L',
      intentsProcessed: 14820,
      ordersWon: 42,
      averageOrderValue: '₹3,380'
    },
    ...
    aiGrowthAgent: {
      ...
      intentInsight: '38% of lost laptop intents were caused by a ₹1,000–₹3,000 price disadvantage.',
      aiRecommendation: 'Apply a maximum 3% dynamic concession to eligible coding-laptop purchase requests above ₹65,000.',
      expectedConversionIncrease: '+14%',
      projectedAdditionalOrders: '+140',
      projectedRevenueImpact: '+₹8.4 Lakhs',
      maxDiscount: '3%',
...
function matchProducts(requirements = {}) {
  ...
  return rfpResults.evaluatedOffers.map(o => ({
    ...
    matchReasons: [
      ...
      `Within budget of ₹${(requirements.budget || 70000).toLocaleString('en-IN')}`,
  ...
  return [
    {
      ...
      specs: 'Bluetooth & 2.4GHz • Silent Click • 18-Month Battery',
    ...
    },
    {
      ...
      specs: 'Adjustable Height • Heat Dissipation Alloy • Foldable',
    ...
    },
    {
      ...
      specs: 'Spill-Resistant 600D Nylon • Velvet Padding • Extra Cable Pocket',
...
function checkBudgetConstraints(budget = 70000, primary = {}, addons = []) {
  ...
  if (isExceeded) {
    ...
    suggestions.push({
      ...
      description: 'Our Bounded Negotiation engine can help you save ₹' + Math.min(2000, exceededBy)
    });
    suggestions.push({
  ...
  return {
    ...
    exceededBy,
    message: isExceeded 
      ? `⚠️ BUDGET EXCEEDED by ₹${exceededBy.toLocaleString('en-IN')}. NEXA Negotiation Engine ready.`
      : `✅ Within budget. Savings: ₹${(budget - totalAmount).toLocaleString('en-IN')}`,
    suggestions
  };
}
