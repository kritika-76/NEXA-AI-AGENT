...
export default function ProductFollowUpChat({
  ...
  const productPrice = Number(product?.price ?? product?.finalTotal ?? 0);
  const productId = product?.id || product?.productId || 'prod_lap_01';
  ...
  useEffect(() => {
    if (product) {
      setMessages([
        {
          sender: 'ai',
          text: `Hi! I'm NEXA. You're asking about the **${productName}**${productPrice ? ` (₹${productPrice.toLocaleString('en-IN')})` : ''}. What would you like to know about its performance, battery, or real-world use?`
        }
      ]);
    }
  ...
  const handleAsk = async (questionText) => {
    ...
    try {
      const res = await aiService.askFollowUp(productId, q, requirements);
      const answer = res?.answer || res?.data?.answer || res?.response?.answer;
      if (!answer) {
        throw new Error('The follow-up response did not include an answer');
      }
      ...
      setMessages(prev => [
        ...
        {
          sender: 'ai',
          text: `I couldn't reach the live AI service right now, but ${productName} is well suited for your requirements. Please try again or ask about performance, battery, upgrades, warranty, or delivery.`
        }
      ]);
    } finally {
      setLoading(false);
            ...
            <div>
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-bold text-slate-900">
                  {productPrice ? `₹${productPrice.toLocaleString('en-IN')}` : 'Selected product'}
                </span>
              </div>
        ...
        <div className="px-4 py-2 bg-slate-50 border-t border-slate-100 flex items-center gap-1.5 overflow-x-auto text-[11px]">
          ...
          {suggestedQuestions.map((sq, idx) => (
            <button
              ...
              type="button"
              disabled={loading}
              className="shrink-0 px-2.5 py-1 bg-white hover:bg-sky-50 disabled:opacity-50 disabled:cursor-not-allowed border border-slate-200 hover:border-sky-200 text-slate-700 rounded-lg font-medium transition-colors cursor-pointer"
            >
              {sq}
            </button>
          ))}
